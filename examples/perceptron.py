...
def perceptron():
    ...
    gui = GUIBuilder()
    mainWindow = gui.mainWindow('perceptron')
    ...
    gui.button(frame, 'reset', lambda:reset(self, 3), 0, 1)
    self.color = gui.optionMenu(frame, ['white', 'red', 'green'], 0, 2)[1]
    gui.label(frame, '', 0, 3, False, True)
    gui.button(frame, 'init' , lambda:init(self), 0, 4)
    gui.button(frame, 'learn', lambda:learn(self), 0, 5)
    self.epochs = gui.optionMenu(frame, ['1', '10', '100', '1000', '10000'], 0, 6)[1]
...
def draw(self, x, y):
    colors = ['white', 'dark red', 'dark green', 'coral', 'light green']
...
def learn(self):
    if len(self.weights) <> 1+2:
        self.weights = [uniform(-1, 1) for l in range(1+2)]
    epochs = int(self.epochs.get())
    for epoch in range(epochs):
        for k in range(self.size):
            for l in range(self.size):
                if self.rects[k][l].color in [1, 2]:
                    train(self.weights, [k / (1.0 * self.size), l / (1.0 * self.size)], self.rects[k][l].color-1)
    for k in range(self.size):
        for l in range(self.size):
            if not(self.rects[k][l].color in [1, 2]):
                self.rects[k][l].color = 3 + calc(self.weights, [k / (1.0 * self.size), l / (1.0 * self.size)])
    display(self)
    
def calc(weights, inputs):
    values = [1] + inputs
    output = sum(weights[k] * values[k] for k in range(len(values)))
    return 0 if output <= 0 else 1

def train(weights, inputs, output):
    error = 0.25 * (output - calc(weights, inputs))
    values = [1] + inputs
    for k in range(len(values)):
        weights[k] = weights[k] + error * values[k]
